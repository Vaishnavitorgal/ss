...
void pass1(char PROGADDR_HEX[10]){
    ...
    while ((fgets(&line, &len, fp1)) != NULL)
    {
        ...
        if(strcmp(record_type, "H") == 0){
            printf("first %s",array[1]);
        	strcpy(control_section_name, array[1]);
        	//starting_address = atoi(array[2]);
        	starting_address = (int)strtol(array[2], NULL, 16);
        	//printf(" starting addr %d",starting_address+90);

        	//printf("\n%d strtol\n",strtol(array[2], NULL, 16));
        	//printf("%d int\n",starting_address);
           	CSLTH = length_of_cs = (int)strtol(array[3], NULL, 16);
            printf("\n%d strtol\n",strtol(array[3], NULL, 16));
        	printf("%d int\n",CSLTH);
...
int main(){
	...
	printf("Enter the program address Starting address: ");
